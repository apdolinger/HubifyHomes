...
import { Loader2, ArrowLeft, CheckCircle, XCircle, Eye, EyeOff, Lock } from 'lucide-react';
...
export default function PortalResetPassword() {
  ...
  const verifyToken = async (tokenValue: string) => {
    try {
      const response = await fetch(`/api/portal/reset-password/verify?token=${encodeURIComponent(tokenValue)}`);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-teal-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 text-center">
          <div className="flex justify-center mb-4">
            <img src={HUBIFY_HOMES_LOGO_URL} alt={HUBIFY_HOMES_LOGO_ALT} className="h-16 w-auto" />
          </div>
