...
function ServicePropertyBadge({ serviceId, count }: { serviceId: number; count: number }) {
  ...
  const trigger = (
    <Badge
      variant="outline"
      className="text-xs border-blue-200 text-blue-700 bg-blue-50 cursor-pointer hover:bg-blue-100 transition-colors min-h-[32px] px-2.5 flex items-center"
    >
  ...
  const assignmentList = (
    <>
      {isLoading ? (
        ...
        <p className="text-sm text-slate-400 px-1">No active assignments found.</p>
      ) : (
        <div className="space-y-1">
          {(assignments as any[]).map((a: any) => {
            const isCustomPrice = a.customPriceCents != null;
            const priceCents: number | null = isCustomPrice
              ? a.customPriceCents
              : (a.serviceDefaultPriceCents ?? null);
            ...
            return (
              <button
                ...
                className="w-full text-left px-3 py-3 rounded-lg hover:bg-slate-100 active:bg-slate-200 transition-colors group min-h-[52px] flex flex-col justify-center"
              >
                <div className="flex items-center justify-between mb-0.5">
                  <span className="text-sm text-slate-800 truncate flex-1 mr-2 font-medium">
