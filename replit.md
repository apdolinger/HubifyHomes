...
- **PDF Mockup Gallery**: Admin-only page at `/admin/pdf-mockups` (linked from Admin > Templates) previewing all 5 client-facing PDFs (Client Invoice, Consolidated Invoice Batch, Inspection Report, Property Report, Time Report) with hardcoded sample data. Each card has an inline HTML preview (yellow "SAMPLE — NOT A REAL CLIENT DOCUMENT" banner + large diagonal SAMPLE watermark) and a "Download Sample PDF" button hitting `GET /api/pdf-mockups/:type` (gated by `isAuthenticated` + `isAdmin`). Server: `server/pdfMockData.ts` (typed sample-data factories), `server/pdfGenerators/index.ts` (shared `streamPdf` + `attachWatermark` adding banner + diagonal SAMPLE on every `pageAdded`), and 5 generators in `server/pdfGenerators/sample*.ts` that accept the sample data via default args. Note: `isAdmin` middleware now resolves user role from the DB for OIDC users (with claims.role as fallback) so role updates take effect on the next request.
