...
- **Core Features**:
    ...
    - **Task Management System**: Priority levels, status tracking, assignment, property association, and due date management. Integrated with calendar. **Recurring Tasks**: Instance-based recurring task generation creates separate task records for each occurrence, enabling individual billing and tracking. Uses RFC5545 RRULE format for recurrence patterns. QuickAddTaskModal includes full recurrence UI with frequency selection, interval, weekday/monthday selection, and end conditions (never, after N occurrences, or on specific date). Recurring tasks display visual Template/Instance badges in the tasks table and task profile. **Task Instance Generation**: Automatic generation triggered on task creation when recurrenceRule is present. Generator respects COUNT and UNTIL parameters in RRULE; for infinite recurrences, generates instances up to 12 months ahead. Each instance receives cloned checklist items with reset completion status and adjusted due dates. Template tasks link to instances via templateTaskId field. **Recurrence Pattern Display**: Template tasks show human-readable recurrence descriptions (e.g., "every week on Monday, Wednesday" or "monthly on the 15th") using RRule's toText() method in both the tasks table and task profile view, making it clear when and how tasks repeat.
    ...
    - **Invoice Management**: Three-tier invoice system with object storage for file upload/download. **Payment Failure Management**: Comprehensive webhook-driven payment tracking with real-time status updates. Stripe webhooks automatically update invoice payment status (pending, processing, succeeded, failed, refunded) and send professional HTML email notifications via SendGrid when payments fail. Failed Payments dashboard widget prominently displays payment issues with color-coded alerts (red for failures, orange for overdue), showing total amounts at risk, payment error details, retry buttons, and overdue calculations. Invoice list displays payment status badges alongside invoice status for at-a-glance visibility. **Professional Invoice Generation**: On-demand PDF generation with organization branding using pdfkit library. PDFs feature professional formatting with company logo, invoice details, itemized line items, and subtotal/total calculations. Stored in object storage at `invoices/org/{orgId}/clients/{clientId}/{invoiceId}.pdf` for persistent access. **Email Delivery System**: Direct invoice sending via SendGrid with professional HTML email templates. Users can compose custom messages and send invoices with attached PDFs to client email addresses. Tracks email delivery with `sentAt` timestamp. Billing UI includes "Generate PDF" button for immediate download and "Send Invoice" button with email composition dialog for customized delivery.
    - **Stripe Integration**: Dual-tier integration for platform subscriptions and per-organization client payments. Organization-level webhook endpoint (`/api/stripe/webhooks/org/:orgId`) handles payment_intent.succeeded, payment_intent.payment_failed, and charge.refunded events with signature verification.
