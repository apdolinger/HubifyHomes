...
- **Property Management Extended**: **Custom Fields**: Custom field support for properties enabling flexible metadata capture with JSONB-backed storage and dynamic UI rendering. **Property Task List**: Property detail pages feature a dedicated Tasks tab that displays all tasks associated with the property, filtered by propertyId. Each task card shows title, description, priority badge, status badge, due date, and assigned user. Tasks are clickable for navigation to task detail pages. Empty state includes a "Create First Task" button that pre-populates the property in the task creation modal. **Organization Supply Settings**: Organization-level customization of supply types and units for property inventory management. Admins can add, remove, and customize supply categories via Admin > Customization tab. Property supply modals dynamically populate dropdowns with organization-specific options. Default supply types include lightbulb, filter, paint, battery, cleaning, hardware, electrical, plumbing, hvac, and other. Default units include piece, gallon, quart, liter, bottle, box, pack, roll, tube, and bag. **Property Access**: Secure storage of property access control information including WiFi passwords, alarm codes, door codes, garage codes, and gate codes. Property detail pages include an Access tab displaying access items grouped by category (WiFi, Alarm, Door, Garage, Gate, Other). Each item stores a description, code/value, and optional notes. Features create, edit, and delete operations with proper authentication (createdById tracking). Access information is stored in the propertyAccessItems table with foreign key linkage to properties. **Room Device Warranty Tracking**: Device tracking system supports warranty management with visual badge indicators. Each room device can have warranty status (hasWarranty boolean), warranty start date, and warranty end date stored as timestamps. Device forms include conditional warranty fields that appear when the warranty checkbox is enabled. Warranty badges display on device cards showing the date range in MM/DD/YYYY format (e.g., "01/15/2024 - 01/15/2029"). Backend handles selective date field updates to preserve existing values during partial updates. Warranty information can be added, edited, or cleared through the device management modal. **Room Device Link Field**: Each room device supports a link field (VARCHAR) for storing URLs to manufacturer pages, product manuals, documentation, or related resources. Links are displayed as clickable elements in the device details modal with ExternalLink icon, opening in new tabs. The link field appears in both the edit modal for data entry and the device details modal for viewing. **Device Details Modal**: Comprehensive read-only device details modal displays all device information organized into sections (Basic Information, Technical Details, Location, Service Information, Warranty Information, Notes). Users can view complete device information by clicking device cards, then switch to edit mode via an "Edit Device" button. The dropdown menu (three dots) on device cards uses event propagation control to prevent accidentally triggering the card click when accessing edit/delete actions. **Device Servicing Schedules**: Room devices support service scheduling with a "Requires Servicing" checkbox that conditionally displays service interval fields (interval number, interval unit dropdown with days/weeks/months/years options, and next service due date). When enabled, the device details modal displays the service schedule as a badge (e.g., "Every 6 months") and includes a "Create Service Task" button that opens the task modal with the property pre-selected, enabling quick creation of service tasks for scheduled device maintenance. Service information is stored in requiresServicing (boolean), serviceInterval (integer), serviceIntervalUnit (varchar), and nextServiceDue (timestamp) fields.
- **Admin Customization**: The Admin page features a Customization tab with secondary tabs for Custom Fields and Supply Settings, consolidating organization-level configuration options in a centralized location for better discoverability and management.
