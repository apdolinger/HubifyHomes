...
- **Task Management**: Priority levels, status tracking, assignment, property association, vendor assignment, due dates, recurring tasks (RFC5545 RRULE), before/after photo categorization, persistent comments, bulk task creation, task templates, context-aware task creation, and custom fields. Vendor assignment feature allows associating tasks with vendors through a "Vendor Needed" checkbox and vendor selection dropdown. Task detail pages display a Vendor Information card when a vendor is assigned, showing vendor name (clickable link to vendor profile), vendor type badge, email, and phone number. Backend storage methods (getTask, updateTask) use Drizzle alias to join vendor contact data for efficient retrieval.
