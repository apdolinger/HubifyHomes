...
- **Super Admin Control Panel**: Internal-only platform management dashboard for Hubify team with 10 specialized tabs including Organizations, Users, Reports, Communication (System Alerts), Revenue, Feature Flags, Monitoring, Platform, Compliance, and Settings. System Alerts support time-based scheduling (start/end dates and times) and location-specific targeting (can be displayed on specific pages like Dashboard, Properties, Tasks, Calendar, Support, etc., or platform-wide). Comprehensive Settings tab provides centralized configuration for platform-wide settings across 9 categories: Platform Configuration (API limits, session timeout, file sizes, webhooks, timezone), Email & Communication (SMTP/SendGrid, SMS gateway, notification defaults), Integration Management (Stripe master keys, OAuth configs, third-party APIs), Default Organization Settings (plan assignment, trial length, storage quotas, feature toggles), Billing & Subscription (pricing tiers, add-ons, grace periods), Compliance & Legal (retention policies, e-signature, GDPR/CCPA, audit logging), System Maintenance (maintenance mode, backup schedules, health checks), Security & Access (2FA policies, password complexity, session controls, IP whitelist), and Customization Controls (global feature toggles, default themes, notification templates).
- **Legal Compliance Pages**: Comprehensive Privacy Policy and Terms of Service pages meeting US and Canadian legal requirements:
  - **Privacy Policy** (/privacy): Full compliance with CCPA/CPRA (California - 11 mandatory elements), GDPR (EU - complete disclosures), and PIPEDA (Canada - 10 fair information principles). Includes detailed sections on data collection, usage, sharing, retention, user rights, security measures, cookies, international transfers, children's privacy, and regulatory contacts.
  - **Terms of Service** (/terms): Complete SaaS legal framework covering acceptance of terms, eligibility (18+ requirement), service description, subscription/payment terms (Stripe integration), license grants, acceptable use policy, intellectual property, privacy integration, service levels, disclaimers/limitations, indemnification, dispute resolution (arbitration), modifications, termination, and general provisions.
  - Both pages are publicly accessible without authentication, with footer links on landing page and authenticated pages.
