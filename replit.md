...
- **Super Admin Control Panel**: Internal dashboard for platform management. The header overview cards (Total Orgs, Total Users, MRR, Uptime), Organizations tab, All Users tab, and Compliance tab are all backed by real DB data — no hardcoded mock numbers. Header reads `/api/super-admin/system-health` (counts.orgs/users/activeSessions, uptimeSeconds) + `/api/super-admin/revenue-metrics` (mrrCents, activeOrgs, pastDueOrgs). Organizations tab reads `GET /api/super-admin/orgs-overview` (orgs joined with subscriptions + user/property counts + primary admin email + MRR cents from platform_settings prices), supports CSV export via `/api/super-admin/orgs-overview.csv`, and Suspend/Activate via `PATCH /api/super-admin/orgs/:orgId/status` (audit-logged). All Users tab reads `GET /api/super-admin/users-overview` with client-side search/role/status filters + CSV export. Compliance tab wires to existing `/api/super-admin/audit-logs`, `/access-review`, `/sessions`; `failedAuth24h` is computed from real audit logs. Removed: orphan "Platform Announcements" TabsContent (not in TabsList) and fake "85% Security Mitigation / SOC 2" hardcoded percentages. Includes organizations, users, reports (communities, vendors with satisfaction ratings), communication, revenue, feature flags, monitoring, and compliance. Revenue/Monitoring/Communication/Settings tabs are backed by real DB data — Revenue aggregates from `org_subscriptions` + plan prices in `platform_settings`; Monitoring shows process metrics (uptime, heap, active sessions, org/user counts) and recent failed Stripe webhooks/notifications; Communication is a full Platform Alerts CRUD (`platform_alerts` + `platform_alert_acknowledgements`) targeted by org/role/location with severity, scheduled start/expiry, ack-required and once-per-session flags, optional action button, surfaced via `GlobalAlertModal` to all authenticated users; Settings reads/writes a `platform_settings` key/value JSONB store (configuration, default plan, plan pricing, maintenance mode/message, password policy, branding, **support_phone** — surfaced in the Hubify Console "Call Support" button and exposed publicly via `/api/support-info`).
